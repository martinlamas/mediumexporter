...
async function loadMediumPost(mediumURL, options = {}) {
  ...
  if (mediumURL.match(/^http/i)) {
    ...
    const res = await r2.get(mediumURL).response
    const resText = await res.text()
    const json = JSON.parse(resText.substr(resText.indexOf('{')))
...
async function processParagraph(p, slug, images, options = {}) {
  ...
  switch (p.type) {
    ...
    case 4: // image & caption
      ...
      images.push(imgsrc)
      let text = options.jekyll
        ? `\n![${p.text}](/assets/images/${slug}/${p.metadata.id})`
        : `\n![${p.text}](images/${sanitize(p.metadata.id)})`
      if (p.text) {
        text += '*' + p.text + '*'
      }
      p.text = text
